...
import MyError from "../utils/myError";
 ...
 export const updateBasketByUserId = async (req: IReq, res: Response, next: NextFunction) => {
        const { foodId } = req.params;
        const{ user } = req;
        ...
        try {
          ...
          console.log("Find IDX: ", findIndex);
          if (findIndex !== -1) {
            const updateBasket = await Basket.findOneAndUpdate(Basket);
          }
          const savedBasket = await (await findBasket.save()).populate("foods.food");
          res.status(200).json({
            message: "Updated сагс.",
            basket: { foods: savedBasket.foods, totalPrice: savedBasket.totalPrice },
          });
        } catch (error) {
          next(error);
